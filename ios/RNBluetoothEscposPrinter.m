...
{
    if(RNBluetoothManagerPrinter.isConnected){
        ...
        [RNBluetoothManagerPrinter writeValue:data withDelegate:self];
    }else{
...
//    escM:{ESC, 'M', 0x00 };
   ...
   // [toSend appendBytes:&NL length:sizeof(NL)];
    ...
    [RNBluetoothManagerPrinter writeValue:toSend withDelegate:delegate];
}
                   ...
                   withResolver:(RCTPromiseResolveBlock) resolve rejecter:(RCTPromiseRejectBlock) reject)
{
    if(RNBluetoothManagerPrinter.isConnected){
        ...
        if((align < 0 || align > 2) && (align < 48 || align > 50)){
            ...
            pendingReject =reject;
            pendingResolve =resolve;
            [RNBluetoothManagerPrinter writeValue:toSend withDelegate:self];
        }
    }else{
         reject(@"COMMAND_NOT_SEND",@"COMMAND_NOT_SEND",nil);
    }
